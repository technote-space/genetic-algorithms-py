...
class Helper:
    ...
    __field: Optional[MutableMapping[int, FieldFlags]] = None
    __count: Optional[int] = None

    @staticmethod
    def position_to_index(x: int, y: int, width: int) -> int:
        return x + width * y
    ...
    @staticmethod
    def get_step_limit() -> int:
        return 800

    @staticmethod
    def __get_foods() -> List[Tuple[int, int]]:
        return [
            (1, 0),
            (2, 0),
            (3, 0),
    ...
    @staticmethod
    def get_field() -> MutableMapping[int, FieldFlags]:
