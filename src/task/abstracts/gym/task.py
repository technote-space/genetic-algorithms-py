...
class AbstractGymTask(AbstractTask):
    """
    Description:
    ------------
    Gym用の学習対象の基底クラス
    ...
    def _get_perceive_function(self, index: int, left: float, right: float) -> Callable[[], bool]:
        return lambda: self.__perceive(self.observation[index], left, right)  # type: ignore
